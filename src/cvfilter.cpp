...
static void icvFilterRow_8u32f(const uchar *src, float *dst, void *params)
{
    ...
    {
        double f = kx[0];
        double s0 = f * icv8x32fTab_cv[(src[i]) + 256], s1 = f * icv8x32fTab_cv[(src[i + 1]) + 256], s2 = f * icv8x32fTab_cv[(src[i + 2]) + 256], s3 = f * icv8x32fTab_cv[(src[i + 3]) + 256];
        for (k = 1, s = src + i + cn; k < ksize; k++, s += cn)
        {
            f = kx[k];
            s0 += f * icv8x32fTab_cv[(s[0]) + 256];
            s1 += f * icv8x32fTab_cv[(s[1]) + 256];
            s2 += f * icv8x32fTab_cv[(s[2]) + 256];
            s3 += f * icv8x32fTab_cv[(s[3]) + 256];
        }
        dst[i] = (float)s0;
        dst[i + 1] = (float)s1;
        dst[i + 2] = (float)s2;
    ...
    for (; i < width; i++)
    {
        double s0 = (double)kx[0] * icv8x32fTab_cv[(src[i]) + 256];
        for (k = 1, s = src + i + cn; k < ksize; k++, s += cn)
            s0 += (double)kx[k] * icv8x32fTab_cv[(s[0]) + 256];
        dst[i] = (float)s0;
    }
}
...
static void icvFilterRow_16s32f(const short *src, float *dst, void *params)
{
    ...
    {
        double f = kx[0];
        double s0 = f * (src[i]), s1 = f * (src[i + 1]), s2 = f * (src[i + 2]), s3 = f * (src[i + 3]);
        for (k = 1, s = src + i + cn; k < ksize; k++, s += cn)
        {
            f = kx[k];
            s0 += f * (s[0]);
...
static void icvFilterRowSymm_8u32f(const uchar *src, float *dst, void *params)
{
    ...
    if (is_symm)
    {
        ...
        {
            double f = kx[0];
            double s0 = f * icv8x32fTab_cv[(s[0]) + 256], s1 = f * icv8x32fTab_cv[(s[1]) + 256], s2 = f * icv8x32fTab_cv[(s[2]) + 256], s3 = f * icv8x32fTab_cv[(s[3]) + 256];
            for (k = 1, j = cn; k <= ksize2; k++, j += cn)
            {
                f = kx[k];
                s0 += f * icv8x32fTab_cv[(s[j] + s[-j]) + 256];
                s1 += f * icv8x32fTab_cv[(s[j + 1] + s[-j + 1]) + 256];
                s2 += f * icv8x32fTab_cv[(s[j + 2] + s[-j + 2]) + 256];
                s3 += f * icv8x32fTab_cv[(s[j + 3] + s[-j + 3]) + 256];
            }
            dst[i] = (float)s0;
            dst[i + 1] = (float)s1;
            dst[i + 2] = (float)s2;
        ...
        for (; i < width; i++, s++)
        {
            double s0 = (double)kx[0] * icv8x32fTab_cv[(s[0]) + 256];
            for (k = 1, j = cn; k <= ksize2; k++, j += cn)
                s0 += (double)kx[k] * icv8x32fTab_cv[(s[j] + s[-j]) + 256];
            dst[i] = (float)s0;
        }
    }
    else
