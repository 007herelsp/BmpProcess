...
#define MAX_CONTOUR_AREA 10000000

void StartSearchProcess(BmpImage *lpSrcImg, set<Box> &lstRes)
{
    ...
    while (contours)
    {
        ...
        if (NULL != result)
        {
            if (SHAPE == result->total)
            {
                ...
                if (dContourArea >= MIN_CONTOUR_AREA && dContourArea <= MAX_CONTOUR_AREA && CheckContourConvexity(result))
                {
                    End_Rage2D = GetMinAreaRect(result);
                    s = 0;
                    Box box;
                    box.box = End_Rage2D;
...
int Process(set<Box> &setURes, BmpImage *lpTargetImg, int argc, char *argv[])
{
    ...
    for (itu = setURes.begin(); itu != setURes.end(); itu++)
    {
        ...
        if (fileIndex < argc)
        {
            BmpImage *lpSrcImg = LoadImage(argv[fileIndex++]);
            if (NULL != lpSrcImg)
            {
                temp = lpSrcImg;
                warp_mat = CreateAutoBuffer(3, 3, VOS_64FC1);
                Point pt;
...
int main(int argc, char **args)
{
    ...
    BmpImage *lpTargetImg = LoadImage(TargetPicPath);
    if (NULL == lpTargetImg)
    {
        return VOS_StsError;
    }
    ...
    const int channel = 3;
    BmpImage *imagChannels[channel] = {NULL};
    ...
    BmpImage *lpCannyImg = CreateImage(GetSize(lpTargetImg), 8, 1);
    BmpImage *lpDilateImg = CreateImage(GetSize(lpTargetImg), 8, 1);
    set<Box> lstRes;
    int iCunt = 0;
    if (NULL != lpCannyImg && NULL != lpDilateImg)
    {
        ...
        BmpImage *lpOutImg = CloneImage(lpTargetImg);
