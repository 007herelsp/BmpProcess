...
void cvStartReadChainPoints(CvChain *chain, CvChainPtReader *reader)
{
    ...
    for (i = 0; i < 8; i++)
    {
        reader->deltas[i][0] = (schar)iCodeDeltas[i].x;
        reader->deltas[i][1] = (schar)iCodeDeltas[i].y;
    }
}

/* retrieves next point of the chain curve and updates reader */
Point cvReadChainPoint(CvChainPtReader *reader)
{
    schar *ptr;
    int code;
    Point pt = {0, 0};
    ...
    if (ptr)
    {
        ...
        assert((code & ~7) == 0);
        reader->pt.x = pt.x + iCodeDeltas[code].x;
        reader->pt.y = pt.y + iCodeDeltas[code].y;
    }
    ...
    for (; y < height; y++, img += step)
    {
        ...
        for (; x < width; x++)
        {
...

            {
                ...
                {
                    int lval = (img0_i ? img0_i[lnbd.y * static_cast<size_t>(step_i) + lnbd.x] : (int)img0[lnbd.y * static_cast<size_t>(step) + lnbd.x]) & 0x7f;
    ...
    for (k = 0; k < 2; k++)
    {
        ...
        for (j = 0; j < contours->total; j++, count++)
        {
            ...
            cvStartWriteSeq(CV_SEQ_ELTYPE_POINT | CV_SEQ_POLYLINE | CV_SEQ_FLAG_CLOSED,
                            contourHeaderSize, sizeof(Point), storage, &writer);
            do
            {
                      ...
                      int maxLevel, Point offset)
{
    ...
    cvDrawContours(&_cimage, &seq[first], color, color, contourIdx >= 0 ? -maxLevel : maxLevel, thickness, lineType, offset);
}
