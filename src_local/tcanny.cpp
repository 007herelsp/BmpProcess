...
int main(int argc, char *argv[])
{
    ...
    IplImage *c_canny;        //
    c_canny = cvCreateImage(cvGetSize(ColorImage), ColorImage->depth, 1);
    ...
    ImgSize size;
    size.cx = OpenCvGrayImage->width;
    size.cy = OpenCvGrayImage->height;
    Canny((unsigned char *)OpenCvGrayImage->imageData, size, 0.1, 0.9, 0.76, (unsigned char *)c_canny->imageData);
    cvNamedWindow("Canny", 0);
    cvShowImage("Canny", c_canny);
